...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    >
        ...
        @if (isset($field['model']))
            <?php $all = isset($field['query']) ? $field['model']::where($field['query']['key'], $field['query']['condition'], $field['query']['value'])->get() : $field['model']::all();?>
            @foreach ($all as $connected_entity_entry)
                <option value="{{ $connected_entity_entry->getKey() }}"
                        @if (
                        (  ! is_null(Request::get($field['name'])) && Request::get($field['name']) == $connected_entity_entry->getKey() )
                        ...
                        || (isset($field['default']) && $field['default'] == $connected_entity_entry->getKey())
                        )
                        selected
                        @endif
                >
